...
export function bsToAD(bsStr: string): string {
  ...
  try {
    const parts = bsStr.trim().split('-').map(Number);
    if (parts.length === 3 && parts[0] > 2000 && parts[0] < 2150 && parts[1] >= 1 && parts[1] <= 12 && parts[2] >= 1 && parts[2] <= 32) {
      const res = bs.toGreg_text(parts[0], parts[1], parts[2]);
      return res || '';
    }
    return '';
  } catch {
