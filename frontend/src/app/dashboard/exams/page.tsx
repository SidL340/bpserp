...
export default function ExamsPage() {
  ...
  const triggerLedgerPrint = () => {
    ...
    printWin.document.write(`
      ...
      <html>
        <head>
          <title>${examTitleStr} Ledger - Brindawan Public School</title>
          <style>
        ...
        <body>
          <div class="header">
            <div class="school-name">बृन्दावन पब्लिक स्कूल, विश्रामपुर, रौतहट (Brindawan Public School)</div>
    ...
    printWin.document.write(`
      ...
      <html>
        ...
        <body>
          <div class="card">
            <div class="header">
              <img src="/school_logo.png" style="width: 54px; height: 54px; object-fit: contain; margin-bottom: 4px;" alt="School Logo" />
  ...
  const triggerBulkGradeSheetsPrint = () => {
    ...
    printWin.document.write(`
      ...
      <html>
        <head>
          <title>Bulk Grade Sheets — Brindawan Public School</title>
          <style>
            @page { size: A4 portrait; margin: 8mm; }
            * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; margin: 0; padding: 0; background: #fff; color: #111; font-size: 11px; }
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl md:text-2xl font-black text-[#0a3d31] tracking-wide uppercase font-serif">
                      {selectedMarksheet.school?.name || SCHOOL_DEFAULTS.name}
                    </h2>
                    {(selectedMarksheet.school?.nameNepali || SCHOOL_DEFAULTS.nameNepali) && (
                      <p className="text-sm font-bold text-gray-700 font-nepali">
                        {selectedMarksheet.school?.nameNepali || SCHOOL_DEFAULTS.nameNepali}
                      </p>
                    )}
                    <p className="text-xs text-gray-600 font-medium">
                      {selectedMarksheet.school?.address || SCHOOL_DEFAULTS.address}
                    </p>
                    <p className="text-[11px] text-gray-500 font-mono">
                      EMIS Code: <strong>{selectedMarksheet.school?.emisCode || SCHOOL_DEFAULTS.emisCode}</strong> • Estd: {selectedMarksheet.school?.estYear || SCHOOL_DEFAULTS.estYear} BS
                    </p>
                  </div>
                </div>

                <div className="pt-2">
                  <span className="inline-block bg-[#0a3d31] text-amber-300 text-xs font-black uppercase px-6 py-1 rounded-full shadow-xs tracking-wider">
                    GRADE-SHEET / PROGRESS REPORT CARD (लब्धाङ्क पत्र)
                  </span>
                </div>
                <p className="text-xs font-extrabold text-[#0a3d31] uppercase tracking-wide pt-1">
                ...
                <div className="flex flex-col items-center justify-center">
                  <div className="relative h-24 w-24 rounded-full border-4 border-double border-[#1e3a5f] flex flex-col items-center justify-center text-center p-1 bg-white/80 shadow-xs transform -rotate-3 transition hover:rotate-0">
                    ...
                    <div className="h-7 w-7 mb-0.5 opacity-90 flex items-center justify-center">
                      {selectedMarksheet.school?.logoUrl ? (
                        ...
                        <svg viewBox="0 0 100 100" className="h-full w-full text-[#1e3a5f]">
                          <polygon points="50,15 61,38 86,38 66,54 74,78 50,62 26,78 34,54 14,38 39,38" fill="#0a3d31" />
                        </svg>
                      )}
                    </div>
                    ...
                    <div className="text-[6.5px] font-bold text-amber-700 tracking-wider">
                      स्था: {selectedMarksheet.school?.estYear || SCHOOL_DEFAULTS.estYear}
                    </div>
                        ...
                        <div className="h-16 w-16 rounded-full overflow-hidden flex items-center justify-center shadow-xs border-2 border-amber-400 bg-white p-1">
                          {bulkMarksheetsData.school?.logoUrl ? (
                            ...
                            <svg viewBox="0 0 100 100" className="h-full w-full text-[#1e3a5f]">
                              ...
                              <text x="50" y="55" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#0a3d31">नेपाल</text>
                            </svg>
                          )}
                        </div>
                        <div>
                          <h2 className="text-xl md:text-2xl font-black text-[#0a3d31] tracking-wide uppercase font-serif">
                            {bulkMarksheetsData.school?.name || SCHOOL_DEFAULTS.name}
                          </h2>
                          {(bulkMarksheetsData.school?.nameNepali || SCHOOL_DEFAULTS.nameNepali) && (
                            <p className="text-sm font-bold text-gray-700 font-nepali">
                              {bulkMarksheetsData.school?.nameNepali || SCHOOL_DEFAULTS.nameNepali}
                            </p>
                          )}
                          <p className="text-xs text-gray-600 font-medium">
                            {bulkMarksheetsData.school?.address || SCHOOL_DEFAULTS.address}
                          </p>
                          <p className="text-[11px] text-gray-500 font-mono">
                            EMIS Code: <strong>{bulkMarksheetsData.school?.emisCode || SCHOOL_DEFAULTS.emisCode}</strong> • Estd: {bulkMarksheetsData.school?.estYear || SCHOOL_DEFAULTS.estYear} BS
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="relative h-24 w-24 rounded-full border-4 border-double border-[#1e3a5f] flex flex-col items-center justify-center text-center p-1 bg-white/80 shadow-xs transform -rotate-3 transition hover:rotate-0">
                          ...
                          <div className="h-7 w-7 mb-0.5 opacity-90 flex items-center justify-center">
                            {bulkMarksheetsData.school?.logoUrl ? (
                              ...
                              <svg viewBox="0 0 100 100" className="h-full w-full text-[#0a3d31]">
                                <polygon points="50,15 61,38 86,38 66,54 74,78 50,62 26,78 34,54 14,38 39,38" fill="#0a3d31" />
                              </svg>
                            )}
                          </div>
                          ...
                          <div className="text-[6.5px] font-bold text-amber-700 tracking-wider">
                            स्था: {bulkMarksheetsData.school?.estYear || SCHOOL_DEFAULTS.estYear}
                          </div>
                          <div className="text-[6px] font-extrabold uppercase bg-[#0a3d31] text-white px-1.5 py-0.5 rounded-full mt-0.5">
                            ★ OFFICIAL SEAL ★
                          </div>
