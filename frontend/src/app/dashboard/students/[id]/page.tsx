...
export default function StudentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const triggerStudentReceiptPrint = () => {
    ...
    const sNameNp = school?.nameNepali || 'बृन्दावन पब्लिक स्कूल';
    const sNameEn = school?.name || 'Brindawan Public School';
    const sAddress = school?.address || 'विश्रामपुर, रौतहट (Brindaban Municipality, Nepal)';
            ...
            <div className="printable-document border-2 border-[#1e3a5f] p-6 rounded-2xl bg-white space-y-4 text-xs font-sans">
              ...
              <div className="flex items-center justify-between border-b-2 border-[#1e3a5f] pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-base font-black text-[#0a3d31] tracking-wide uppercase">
                      {school?.name || SCHOOL_DEFAULTS.name}
                    </h2>
                    <h3 className="text-xs font-bold text-gray-800 font-nepali">
                      {school?.nameNepali || SCHOOL_DEFAULTS.nameNepali}
                    </h3>
                    <p className="text-[10px] text-gray-600">
                      {school?.address || SCHOOL_DEFAULTS.address} • EMIS: <b className="font-mono">{school?.emisCode || SCHOOL_DEFAULTS.emisCode}</b>
                    </p>
                    <p className="text-[10px] text-gray-600">Phone: {school?.phone || SCHOOL_DEFAULTS.phone}</p>
                  </div>
                </div>

                <div className="text-right space-y-1">
                  <span className="inline-block bg-[#0a3d31] text-white font-extrabold px-3 py-1 rounded text-xs tracking-wide">
                    JOURNAL FEE RECEIPT
                  </span>
                  <p className="font-mono font-bold text-sm text-[#0a3d31] mt-1">No: {selectedReceipt.receiptNo}</p>
