...
const navConfig: (NavItem | NavSection)[] = [
  ...
  },
  {
    ...
    items: [
      { label: 'Admissions', nepaliLabel: 'भर्ना आवेदन', href: '/dashboard/admissions', icon: UserPlus, roles: ['SUPER_ADMIN', 'ADMIN', 'ACCOUNTANT'] },
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active
          ? 'bg-gradient-to-r from-amber-400 to-amber-300 text-[#072a22] shadow-md font-bold'
          : 'text-slate-200 hover:bg-[#0e4437] hover:text-white',
    ...
    >
      <Icon
        ...
        className={cn(
          'shrink-0',
          active ? 'text-[#072a22]' : 'text-emerald-200/70 group-hover:text-white'
        )}
      />
      {!collapsed && (
        ...
        return (
          <li key={idx} className="pt-2">
            {!collapsed ? (
              ...
              <div className="mx-auto my-2 h-px w-6 bg-[#0e3d32]" />
            )}
...
export function Sidebar({ mobileOpen = false, onMobileClose = () => {} }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'hidden md:flex md:shrink-0 md:flex-col bg-[#072a22] transition-all duration-200 border-r border-[#0e3d32]',
          collapsed ? 'w-20' : 'w-64'
        )}
      >
        {/* Logo Header */}
        <div
          className={cn(
            'flex items-center border-b border-[#0e3d32] px-4 py-4',
            collapsed ? 'justify-center px-2' : 'gap-3'
          )}
        >
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white p-1 shadow-md ring-2 ring-amber-400/50">
            <img src="/school_logo.png" alt="School Emblem Seal" className="w-9 h-9 object-contain rounded-full" />
          </div>
          {!collapsed && (
            <div className="min-w-0">
              <p className="truncate text-sm font-black text-white tracking-wide">BPS ERP</p>
              <p className="truncate text-[10px] font-semibold text-amber-300 font-nepali">बृन्दावन पब्लिक स्कूल</p>
            </div>
          )}
        </div>
        ...
        <div className="border-t border-[#0e3d32] p-2 bg-[#05201a]">
          <button
            onClick={() => setCollapsed(!collapsed)}
            className="mb-1.5 hidden w-full items-center justify-center rounded-lg py-1.5 text-emerald-300/70 transition hover:bg-[#0e3d32] hover:text-white md:flex"
          ...
          >
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-amber-400 to-amber-300 text-xs font-black text-[#072a22] shadow-sm">
              {initials || 'U'}
            </div>
            {!collapsed && (
              <div className="min-w-0 flex-1">
                <p className="truncate text-xs font-bold text-white">{displayName}</p>
                <p className="truncate text-[10px] uppercase font-semibold text-amber-300/90">{role}</p>
              </div>
            )}
            <button
              onClick={handleLogout}
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-emerald-300/70 transition hover:bg-rose-500/20 hover:text-rose-300"
              title="Logout"
            >
              <LogOut size={15} />
            </button>
          ...
          {!collapsed && (
            <div className="mt-1 pt-1.5 border-t border-[#0e3d32] text-[9px] text-center text-emerald-200/60">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          <aside className="absolute inset-y-0 left-0 flex w-72 flex-col bg-[#072a22] shadow-2xl z-50">
            <div className="flex items-center justify-between border-b border-[#0e3d32] px-4 py-4">
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white p-1 shadow-md ring-2 ring-amber-400/50">
                  <img src="/school_logo.png" alt="School Emblem Seal" className="w-9 h-9 object-contain rounded-full" />
                </div>
                <div>
                  <p className="text-sm font-black text-white">BPS ERP</p>
                  <p className="text-[10px] font-semibold text-amber-300 font-nepali">बृन्दावन पब्लिक स्कूल</p>
                </div>
              </div>
              <button
                onClick={onMobileClose}
                className="flex h-8 w-8 items-center justify-center rounded-lg text-emerald-300/80 hover:bg-[#0e3d32] hover:text-white"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="border-t border-[#0e3d32] p-3 bg-[#05201a]">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-amber-400 to-amber-300 text-xs font-black text-[#072a22]">
                  {initials || 'U'}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-bold text-white">{displayName}</p>
                ...
                <button
                  onClick={handleLogout}
                  className="flex h-8 w-8 items-center justify-center rounded-md text-emerald-300/70 hover:bg-rose-500/20 hover:text-rose-300"
                >
                  <LogOut size={16} />
                </button>
              </div>
