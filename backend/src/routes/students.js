...
router.post('/', authenticate, authorize('SUPER_ADMIN', 'ADMIN'), async (req, res) => {
  try {
    const {
      fullName, fullNameNepali, gender, dateOfBirthBs, dateOfBirthAd, address, phone,
      ...
      religion, ethnicity, disability, admissionChargeType = 'CHARGEABLE',
      admissionFeeAmount, transportFeeAmount, transportStop,
      scholarshipType = 'NONE', discountPercent = 0, customMonthlyFee, discountRemarks,
    } = req.body;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const student = await tx.student.create({
        data: {
          ...
          address, phone, fatherName, motherName, guardianName,
      ...
      // Auto-generate Admission Fee due (if chargeable)
      ...
      if (isChargeable) {
        ...
        let feeAmount = admissionHead?.amount || 2000;
        if (admissionFeeAmount !== undefined && admissionFeeAmount !== '') {
          feeAmount = parseFloat(admissionFeeAmount);
        } else if (classId && admissionHead) {
        ...
        if (feeAmount > 0) {
          await tx.studentFeeDue.create({
            data: {
              ...
              amount: feeAmount,
              dueDateBs: admissionDateBs || new Date().toISOString().slice(0, 10),
              remarks: 'Admission Fee on Enrollment',
            },
          }).catch(() => {});
        }
      }
      ...
      if (transAmount > 0) {
        let transportHead = await tx.feeHead.findFirst({
          where: { OR: [{ name: { contains: 'Transport' } }, { nameNepali: { contains: 'यातायात' } }], isActive: true },
        });
        if (!transportHead) {
        ...
        await tx.studentFeeDue.create({
          data: {
            ...
            amount: transAmount,
            dueDateBs: admissionDateBs || new Date().toISOString().slice(0, 10),
            remarks: `Monthly Transportation Fee (${transportStop || 'Bus Route'})`,
          },
        }).catch(() => {});
      }
...
router.put('/:id', authenticate, authorize('SUPER_ADMIN', 'ADMIN'), async (req, res) => {
  try {
    const data = { ...req.body };
    if (data.dateOfBirthAd !== undefined) {
      data.dateOfBirthAd = data.dateOfBirthAd ? new Date(data.dateOfBirthAd) : null;
    }
    // Don't accidentally try to write relations or non-model fields
    delete data.id;
    delete data.user;
    delete data.classEnrollment;
    delete data.attendances;
    delete data.feeCollections;
    delete data.libraryIssues;
