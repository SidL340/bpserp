...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    if (!classId || !dateBs || !entries) {
      return res.status(400).json({ success: false, message: 'classId, dateBs, and entries are required.' });
    }

    const adDate = dateAd ? new Date(dateAd) : new Date();
    const results = [];
    ...
    for (const entry of entries) {
      ...
      // Auto-notice & SMS on ABSENT
      if (entry.status === 'ABSENT') {
        const student = await prisma.student.findUnique({
          where: { id: entry.studentId },
          select: { fullName: true, guardianContact: true, guardianName: true },
        });
        const noticeText = `Dear Parent, your child ${student?.fullName || 'Student'} was marked ABSENT at Brindawan Public School on ${dateBs}. If this was unexpected, please contact the school.`;
        ...
        if (student?.guardianContact) {
          sendSparrowSms(student.guardianContact, noticeText).catch(console.error);
        }
      }
    }
