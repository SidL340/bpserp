...
// File upload configuration for school gallery and branding assets
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    const uniqueName = `bps-${cleanBase}-${Date.now()}${ext}`;
    cb(null, uniqueName);
  },
});
const upload = multer({
...
// POST /api/school/upload-image — Upload photo for gallery, logo, or banner
router.post('/upload-image', authenticate, authorize('SUPER_ADMIN', 'ADMIN'), upload.single('image'), (req, res) => {
  try {
    if (!req.file) {
...
// GET /api/school/gallery — Fetch school photo gallery
router.get('/gallery', async (req, res) => {
  try {
    const school = await prisma.school.findFirst({ select: { galleryJson: true } });
    let gallery = [];
    if (school?.galleryJson) {
      try {
        gallery = JSON.parse(school.galleryJson);
      } catch (e) {
        gallery = [];
      }
    }
...
// POST /api/school/gallery — Save / update school photo gallery
router.post('/gallery', authenticate, authorize('SUPER_ADMIN', 'ADMIN'), async (req, res) => {
  try {
    const { gallery } = req.body;
    const existing = await prisma.school.findFirst();
    const jsonString = typeof gallery === 'string' ? gallery : JSON.stringify(gallery || []);
    const updated = await prisma.school.update({
      where: { id: existing ? existing.id : 1 },
      data: { galleryJson: jsonString },
    });
    return res.json({ success: true, message: 'School gallery updated successfully', data: updated });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
});
