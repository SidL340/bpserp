...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    if (role === 'STUDENT') {
      ...
      const activeEnrollment = sId
        ? await prisma.classEnrollment.findFirst({ where: { studentId: sId, isActive: true } })
        : null;
      
      const studentConditions = [
        // 1. Notice targeted specifically to this student
        ...(sId ? [{ targetStudentId: sId }] : []),
        // 2. Notice targeted specifically to this student's class (and not to any other individual student)
        ...(activeEnrollment?.classId ? [{ targetClassId: activeEnrollment.classId, targetStudentId: null }] : []),
        // 3. School-wide general notices (must not be targeted to an individual student or specific class)
        {
